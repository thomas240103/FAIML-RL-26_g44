...
\bottomrule
\end{tabular}
\caption{Friction-bin success rates for \texttt{SAC trained on the hard target (UDR using full distribution)} and \texttt{SAC using ADR} on the hard target deterministic evaluation, aggregated over 150 episodes using 50 episodes for each seed in $\{0,42,99\}$. The hard target uses the same mass and friction randomization protocol as Table~\ref{tab:nominal-eval}.}
