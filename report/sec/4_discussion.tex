...
The most counter-intuitive result is that the ADR source model (94\%) outperforms the UDR hard target model (72\%) despite the latter being trained on a distribution that exactly matches the evaluation distribution.
A natural first hypothesis is that certain mass--friction combinations are physically beyond the arm's capability: the Panda joints have limited torques (87~N on most joints), and pushing a 10~kg object against high friction may be infeasible regardless of the policy.
However, this hypothesis does not explain the gap, because both models face the same hard cases during evaluation.
