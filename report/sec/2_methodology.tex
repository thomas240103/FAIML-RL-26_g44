...
The second part relies on the Stable-Baselines3 implementations of two state-of-the-art actor-critic algorithms. PPO~\cite{schulman2017ppo} is an \textbf{on-policy} method that collects rollouts under the current policy and updates it by maximizing a clipped surrogate objective,
\begin{equation}
...
\midrule
Environment & \textbf{source} & source \\
Sampling strategy & \textbf{\texttt{none}} & \textbf{\texttt{none}} \\
Training steps & \textbf{500k} & \textbf{500k} \\
Policy & \textbf{\texttt{MultiInputPolicy}} & \textbf{\texttt{MultiInputPolicy}} \\
Learning rate & \textbf{0.001}, $3e^{-4}$, $10^{-4}$ & $10^{-3}$, \textbf{2e-4}, $3e^{-4}$, $10^{-4}$ \\
Discount $\gamma$ & \textbf{0.95}, 0.98, 0.99 & \textbf{0.95}, 0.98, 0.99 \\
Batch size & 512, 1024, \textbf{2048} & \textbf{64} \\
Gradient steps / epochs & \textbf{-1} & \textbf{10} \\
Rollout steps & -- & \textbf{2048} \\
Learning starts & \textbf{100k} & -- \\
Replay buffer size & \textbf{10$^6$} & -- \\
Target update $\tau$ & \textbf{0.05} & -- \\
Entropy coefficient & \textbf{auto} & \textbf{0.0} \\
GAE $\lambda$ & -- & \textbf{0.95} \\
Clip range & -- & \textbf{0.2} \\
Policy network & \textbf{default SB3} & \textbf{[256, 256]} \\
\bottomrule
\end{tabular}
}
...
Bold values indicate the final parameters selected for the evaluation models.
}
...
The wrapper \texttt{part2/rand\_wrapper.py} implements three modes. In \texttt{none}, the nominal environment dynamics are kept. In \texttt{udr}, the wrapper samples a new mass and friction values at every reset. The UDR training ranges are:
\begin{equation}
